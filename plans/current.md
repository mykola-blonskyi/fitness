...
- [x] Generation draws only from Food Items that carry a Family (no rule needed for sweets/beverages/flours/offal — they simply have none)
...
- [x] Reclassification: potato + sweet potato -> `complex_carb`/`starchy_vegetable`, beans + lentils -> `plant_protein`, olives -> fat; Category untouched
...
- [x] Free Foods: the `salad_vegetable` and `cooked_vegetable` Families get fixed nominal portions, leave the macro fit and the displayed totals, and what they actually supply is subtracted from all four targets before fitting (FITNESS-74)
  ...
  - The 80 g/15 g bulk-vs-accent split and the favourites question deferred from here are both closed by FITNESS-75 (ADR-021): `accent_vegetable` is a Family of its own at 15 g, and favourites narrow by Family rather than bias. Neither shipped as a bias.
  - Deploy gate: production's `family_id` was still null everywhere when this was written, and the 100 g vegetable floor is gone. The generation gate is not scoped to vegetable Roles — `findCandidatesByRole` drops every candidate with no Family — so against an unclassified catalog generation returns 422 for every user rather than producing a vegetable-less menu. The boot-time pass above closes this on the next deploy; FITNESS-76 tracks confirming it against production.
