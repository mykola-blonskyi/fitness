...
- [x] Curated staples set (~80-120 foods: macros, family, names in all four locales), delivered as a reviewable file **before** seeding
  ...
  - The 8 dairy staples (cottage cheese, greek yogurt, kefir, milk) are **not reachable by generation**, and neither are the 26 inherited `dairy` rows. No chain in `MEAL_ROLE_CHAINS` contains the `dairy` Role, and keeping that Role is what ADR-020 requires so "exclude dairy" still works. They become reachable when Phase 2's dinner Archetype draws `casein_dairy` by Family instead of by Role. Until then the ticket's headline foods are seeded but unused.
