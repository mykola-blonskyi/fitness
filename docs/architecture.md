...
- App-wide nav header (see [ADR-007](docs/decisions.md)) — a nav menu scoped to built sections, not a breadcrumb trail, since fitness has several sibling top-level sections a user moves *between* rather than a single hierarchy to track depth within. Renders everywhere except `/onboarding`; includes a language switcher (FITNESS-11) — theme toggling is still deliberately not part of it
